...
export class SessionComponent implements OnInit, OnDestroy {
  ...
  public get session() { return this.sessionService.currentSession; }
  ...
  public get slots() {
    return _(this.session.slots).sortBy(s => s.time).value();
  }

  public get rooms() {
    return _(this.session.rooms).orderBy(r => r ? r.seats : 0, ['desc']).value();
  }
  ...
  private onTopicMoveEnd(event) {
    if (event.interaction.dropTarget == null) {
      this.updateTarget(document.querySelector('.topics-unassigned'), event.target);
    }
  }
  ...
  private onTopicDrop(event) {
    const isSwappingTopics = event.target.children.length > 0
      && !event.target.classList.contains('topics-unassigned');

    if (isSwappingTopics) {
      const currentTopicElement = event.target.children[0].children[0];
      const relatedTargetParent = event.relatedTarget.parentElement.parentElement;
  ...
  private getElementRoom(element: Element) {
    try {
      ...
      return document.querySelector('.session-table thead tr').children[index].getAttribute('id');
    } catch {
      return null;
    }
  }
